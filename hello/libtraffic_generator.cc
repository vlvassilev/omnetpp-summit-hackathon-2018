...
traffic_generator_instance_t traffic_generator_init(const char* config_str)
{
    tg_t* tg;
    tg = (tg_t*)malloc(sizeof(tg_t));
    memset(tg,0,sizeof(tg_t));
    tg->config=strdup(config_str);
    return (traffic_generator_instance_t)tg;
}
#define PACKET_LEN 64
#define INTERFRAME_GAP 64
int traffic_generator_get_frame(traffic_generator_instance_t tg_id, uint32_t* len, uint8_t** data, uint64_t* tx_time_sec, uint32_t* tx_time_nsec)
{
    uint8_t* data_buf;
    tg_t* tg;
    tg = (tg_t*)tg_id;
    data_buf=(uint8_t*)malloc(PACKET_LEN);
    memset(data_buf,0,PACKET_LEN);
    *len=PACKET_LEN;
    *data=data_buf;
